...
--!! create indexes
create HG index clientid_HG on #client_Zp(clientid);commit;
create HG index ZpOKPO_HG on #client_Zp(ZpOKPO);commit;

-- *по окпо предприятия от которого зачисления дотянуть код сферы деятельности из табл rm.TCLIENTINFO (поле K110) , 
--связка по ZpOKPO и inn
     SELECT t3.* --add alias
          , T4.k110 --alias change t5 --> t4
       INTO #client_Zp_k110
...
--* по коду сферы деятельности вытянуть из справочника название сферы деятельности (табл. rm.k110, поле UKR_SUB_INDUST)
SELECT t5.* --add alias
     , T6.UKR_SUB_INDUST --alias change t5 --> t4
  INTO #client_Zp_INDUST_k110
  FROM #client_Zp_k110 AS T5 --change table name
...
--V2 лучше не делать , т.к. большие табл
SELECT *
     , T2.k110
     , T3.UKR_SUB_INDUST
  INTO #client_Zp
...
--MIN, MAX, AVG, SUM игнорируют значения NULL
...
SELECT  *,
        cast(null as varchar(10)) as k110,
        cast(null as varchar(255)) as UKR_SUB_INDUST
  INTO #client_Zp
  FROM rm.tDebCard AS T1
 WHERE AvgZpObj > 0 -- OR IS NOT NULL
   AND ZpProjType IN (SELECT ZpProjType
...
update #client_Zp as t1
set t1.k110 = t2.k110
FROM #client_Zp AS t1
JOIN rm.TCLIENTINFO AS t2 ON t1.ZpOKPO = t2.inn
;
commit;

update #client_Zp as t1
set t1.UKR_SUB_INDUST = t2.UKR_SUB_INDUST
FROM #client_Zp AS t1
JOIN rm.k110 AS t2 ON t1.k110 = t2.k110
;
commit;
